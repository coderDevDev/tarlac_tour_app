...
export default function ARCameraPage() {
  ...
  const startQrScanning = async () => {
    ...
    try {
      ...
      setError("Failed to initialize QR scanner. Please try uploading an image instead.")
      setScanning(false)
    }
  }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError("Failed to process the image. Please try another image or use the camera scanner.")
    } finally {
      setIsLoading(false)
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button variant="outline" onClick={() => router.push("/ar-camera")} className="flex items-center gap-2">
            <X className="h-4 w-4" />
            Close AR
          </Button>
        </div>
      ...
      <AnimatePresence mode="wait">
        {!cameraActive ? (
          ...
          >
            ...
            {error && (
              <div className="mt-4 text-sm text-red-500 text-center px-4 py-2 bg-red-50 rounded-md">{error}</div>
            )}
          </motion.div>
        )}
      </AnimatePresence>
